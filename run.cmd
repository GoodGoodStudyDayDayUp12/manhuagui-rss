...
node "%~dp0gen-e.mjs" --out feed-e.xml --limit 50 --with-content --content-limit 20 --guid-version 2 --self "https://cdn.jsdelivr.net/gh/%GHUSER%/%GHREPO%@main/feed-e.xml"
if errorlevel 1 echo    [warn] e failed
node "%~dp0gen-f.mjs" --out feed-f.xml --limit 30 --self "https://cdn.jsdelivr.net/gh/%GHUSER%/%GHREPO%@main/feed-f.xml"
if errorlevel 1 echo    [warn] f failed
node "%~dp0gen-g.mjs" --out feed-g.xml --limit 100 --with-content --content-limit 20 --guid-version 1 --self "https://cdn.jsdelivr.net/gh/%GHUSER%/%GHREPO%@main/feed-g.xml"
if errorlevel 1 echo    [warn] g failed
node "%~dp0gen-d.mjs" --mid 3493278460676126 --title "订阅源 H" --out feed-h.xml --limit 30 --self "https://cdn.jsdelivr.net/gh/%GHUSER%/%GHREPO%@main/feed-h.xml"
if errorlevel 1 echo    [warn] h failed
node "%~dp0gen-i.mjs" --config config-i.json
if errorlevel 1 echo    [warn] i failed
